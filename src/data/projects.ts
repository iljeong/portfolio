...
export const projects: Project[] = [

  {
    ...
    content: {
      problem:
        '새로운 강사 영입 시 콘텐츠 방향, 타겟, 커리큘럼 구성, 페이지 기획이 담당자 경험에만 의존하고 있었다. 제작 프로세스가 문서화되지 않아 일정 지연과 품질 편차가 반복됐다. 8개월 인턴 기간 동안 기획부터 런칭까지의 전 주기를 6회 반복한, 이 포트폴리오의 유일한 실무 경험이자 출발점이다.',
      approach: [
  ...
  },
  {
    ...
    content: {
      problem:
        'Gcar 앱의 부정 리뷰가 반복되고 있었으나, 어떤 문제가 핵심 이탈을 유발하는지 구조적으로 파악된 데이터가 없었다. 개별 이슈 대응에 그칠 뿐, 근본 원인을 고객 여정 관점으로 정의한 시도가 없었다. 패스트캠퍼스에서 데이터로 기획 방향을 수정했던 방식을, 분석 도구를 갖춰 3.8만 건 규모로 확장한 프로젝트다.',
      approach: [
